...
[Pool(typeof(TheApprenticeBCardPool))]
public abstract class ApprenticeCardB(
    ...
    : ConstructedCardModel(cost, type, rarity, target, showInCardLibrary)
{
    ...
    // Call in subclass constructors to register the dynamic Intense hover tip.
    // Mirrors ApprenticeCard.WithDreamKeywordTips() pattern. The lambda is evaluated at
    // hover time with the live card, so it reads the current IntenseModifier.Stacks —
    ...
    protected void WithIntenseTip()
    {
        WithTips(card =>
        {
            ...
            {
                new HoverTip(
                    new LocString("card_keywords", "THEAPPRENTICE-INTENSE.title"),
    ...
    public override Task BeforeCombatStart()
    {
        ...
        if (Keywords.Contains(ApprenticeKeywords.Stable))
    ...
    // Performance queues cards. Mirrors ApprenticeCard.AfterPlayerTurnStartLate.
