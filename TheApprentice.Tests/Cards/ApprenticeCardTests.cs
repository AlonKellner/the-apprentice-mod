...
public class ApprenticeCardTests
{
    ...
    [Fact] public void Realize_IsAttack_Common_NotPrePlanned() { var c = new Realize(); Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.False(c.IsPrePlanned); }
    [Fact] public void Epiphany_IsSkill_Common_NotPrePlanned_Expend() { var c = new Epiphany(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Common, c.Rarity); Assert.False(c.IsPrePlanned); Assert.DoesNotContain(c.Keywords, k => k == CardKeyword.Exhaust); Assert.Contains(c.Keywords, k => k == ApprenticeKeywords.Expend); }
    ...
    [Fact] public void Preface_IsAttack_Common_0Cost()  { var c = new Preface();   Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Basic,   c.Rarity); Assert.Equal(0, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Foresight_IsSkill_Uncommon_1Cost() { var c = new Foresight(); Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void CreativeBlock_IsSkill_Uncommon_NotPrePlanned() { var c = new CreativeBlock(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.False(c.IsPrePlanned); }
    [Fact] public void Rehearsal_IsSkill_Uncommon_NotPrePlanned() { var c = new Rehearsal(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.False(c.IsPrePlanned); }
    ...
    [Fact] public void Nocturne_IsSkill_Common()    { var c = new Nocturne();   Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic,   c.Rarity); Assert.False(c.IsPrePlanned); }
    ...
    [Fact] public void Longing_IsSkill_Common()     { var c = new Longing();    Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic,   c.Rarity); Assert.False(c.IsPrePlanned); }
    ...
    [Fact] public void Manifesto_IsSkill_Uncommon()          { var c = new Manifesto();        Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); }
    [Fact] public void Lucidity_IsSkill_Uncommon()           { var c = new Lucidity();         Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); }
    [Fact] public void Resolve_IsSkill_Uncommon()            { var c = new Resolve();          Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); }
    [Fact] public void Ignition_IsSkill_Uncommon()           { var c = new Ignition();         Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); }
    [Fact] public void Sublimation_IsSkill_Uncommon()        { var c = new Sublimation();      Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); }
    ...
    [Fact] public void Daydream_IsSkill_Rare_Expend()   { var c = new Daydream();    Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Rare, c.Rarity); Assert.DoesNotContain(c.Keywords, k => k == CardKeyword.Exhaust); Assert.Contains(c.Keywords, k => k == ApprenticeKeywords.Expend); }
    [Fact] public void Wunderkind_IsPower_Rare() { var c = new Wunderkind();  Assert.Equal(CardType.Power,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); }
    ...
    [Fact] public void Solace_IsSkill_Common_1Cost()    { var c = new Solace();    Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic,   c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Canticle_IsAttack_Common_1Cost()   { var c = new Canticle();    Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Repose_IsSkill_Common_1Cost()      { var c = new Repose();      Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Candor_IsSkill_Common_1Cost()      { var c = new Candor();      Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Inversion_IsSkill_Common_2Cost_Expend() { var c = new Inversion(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(2, c.EnergyCost.Canonical); Assert.DoesNotContain(c.Keywords, k => k == CardKeyword.Exhaust); Assert.Contains(c.Keywords, k => k == ApprenticeKeywords.Expend); }
    [Fact] public void Exertion_IsSkill_Common_0Cost()  { var c = new Exertion();  Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(0, c.EnergyCost.Canonical); }
    [Fact] public void Strain_IsSkill_Common_0Cost()    { var c = new Strain();    Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(0, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Dissolution_IsSkill_Uncommon_1Cost()  { var c = new Dissolution();  Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Brace_IsSkill_Uncommon_0Cost()        { var c = new Brace();        Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(0, c.EnergyCost.Canonical); }
    [Fact] public void Burden_IsAttack_Uncommon_1Cost()       { var c = new Burden();       Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Ensemble_IsSkill_Uncommon_1Cost()      { var c = new Ensemble();     Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Discord_IsAttack_Uncommon_2Cost()      { var c = new Discord();      Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(2, c.EnergyCost.Canonical); }
    [Fact] public void Release_IsAttack_Uncommon_1Cost()      { var c = new Release();      Assert.Equal(CardType.Attack,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Reflection_IsSkill_Uncommon_2Cost_Expend() { var c = new Reflection(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(2, c.EnergyCost.Canonical); Assert.DoesNotContain(c.Keywords, k => k == CardKeyword.Exhaust); Assert.Contains(c.Keywords, k => k == ApprenticeKeywords.Expend); }
    [Fact] public void Unburdening_IsSkill_Uncommon_0Cost()   { var c = new Unburdening();  Assert.Equal(CardType.Skill,   c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(0, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Defiance_IsSkill_Uncommon_1Cost()      { var c = new Defiance();     Assert.Equal(CardType.Skill,   c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Tirade_IsAttack_Rare_2Cost()            { var c = new Tirade();     Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(2, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Penance_IsPower_Rare_2Cost()            { var c = new Penance();       Assert.Equal(CardType.Power, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(2, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Diminuendo_IsSkill_Uncommon_1Cost() { var c = new Diminuendo(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Flat_IsSkill_Common_1Cost()         { var c = new Flat();       Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Basic,   c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Sforzando_IsSkill_Uncommon_0Cost()  { var c = new Sforzando();  Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(0, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Motif_IsSkill_Common_1Cost()     { var c = new Motif();    Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Strings_IsSkill_Common_1Cost()   { var c = new Strings();  Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Tremolo_IsSkill_Common_1Cost()   { var c = new Tremolo();  Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Score_IsSkill_Uncommon_2Cost()   { var c = new Score();    Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(2, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Refrain_IsSkill_Common_1Cost()            { var c = new Refrain();         Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic,   c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Attunement_IsSkill_Uncommon_1Cost()       { var c = new Attunement();      Assert.Equal(CardType.Skill,  c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Flourish_IsAttack_Uncommon_1Cost()        { var c = new Flourish();         Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Basic, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    ...
    [Fact] public void Transcendence_IsSkill_Rare_1Cost_Expend()  { var c = new Transcendence();  Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Rare, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); Assert.DoesNotContain(c.Keywords, k => k == CardKeyword.Exhaust); Assert.Contains(c.Keywords, k => k == ApprenticeKeywords.Expend); }
    ...
    // ── New cards (must fail until card files are created) ─────────────────────

    [Fact] public void Overtone_IsSkill_Common_1Cost() { var c = new Overtone(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Common, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); }
    [Fact] public void Resonance_IsSkill_Uncommon_1Cost_Expend() { var c = new Resonance(); Assert.Equal(CardType.Skill, c.Type); Assert.Equal(CardRarity.Uncommon, c.Rarity); Assert.Equal(1, c.EnergyCost.Canonical); Assert.DoesNotContain(c.Keywords, k => k == CardKeyword.Exhaust); Assert.Contains(c.Keywords, k => k == ApprenticeKeywords.Expend); }
    [Fact] public void Cadenza_IsAttack_Rare() { var c = new Cadenza(); Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Rare, c.Rarity); }
    [Fact] public void Elegy_IsAttack_Common_0Cost() { var c = new Elegy(); Assert.Equal(CardType.Attack, c.Type); Assert.Equal(CardRarity.Common, c.Rarity); Assert.Equal(0, c.EnergyCost.Canonical); }
}
