...
public class SellOut : UnderstudyCard
{
    public const string CardId = "TheUnderstudy:SellOut";

    public SellOut() : base(1, CardType.Attack, CardRarity.Uncommon, TargetType.AnyEnemy)
    {
        WithDamage(5);
    ...
    protected override async Task OnPlay(PlayerChoiceContext context, CardPlay cardPlay)
    {
        ...
        // while sitting in the exhaust pile (IsCombatPile covers Exhaust), so Showtime/
        // Workshop/DaCapo can still find and play it later via PlannedModifier.RelevantCards.
        Log.Info($"SellOut.OnPlay: exhausting {plannedCards.Count} Planned card(s), keeping their Planned status");
