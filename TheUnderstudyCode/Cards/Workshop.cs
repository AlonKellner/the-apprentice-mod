...
public class Workshop : PlayAllPlannedCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext context, CardPlay cardPlay)
    {
        ...
        foreach (var (card, _, slotSeqIdx) in planned)
        {
            ...
            if (card.Pile == null)
            {
                Log.Info($"Workshop.OnPlay: {card.Id} is no longer in any pile — skipped");
                continue;
            }
            ...
            if (card.TargetType == TargetType.AnyEnemy && (currentTarget == null || currentTarget.IsDead))
            {
                ...
                Log.Info($"Workshop.OnPlay: target {previousTarget?.LogName ?? "(none)"} is no longer available — " +
        ...
        var selectedRaw = await CardSelectCmd.FromCombatPile(
            ...
            player,
            new CardSelectorPrefs(new LocString("cards", "THEUNDERSTUDY-WORKSHOP.selectionPrompt"), 0, maxSelect),
