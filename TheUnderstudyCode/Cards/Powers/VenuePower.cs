...
public class VenuePower : UnderstudyPower
{
    ...
    public override async Task AfterSideTurnEnd(PlayerChoiceContext context, CombatSide side, IEnumerable<Creature> participants)
    {
        ...
        Log.Info($"VenuePower.AfterSideTurnEnd: no card played this turn — auto-playing {planned.Count} Planned slot(s)");
        ...
        foreach (var (card, _, slotSeqIdx) in planned)
        {
            if (card.Pile == null)
            {
                Log.Info($"VenuePower.AfterSideTurnEnd: {card.Id} is no longer in any pile — skipped");
                continue;
            }
            ...
            if (card.TargetType == TargetType.AnyEnemy && (currentTarget == null || currentTarget.IsDead))
            {
                ...
                Log.Info($"VenuePower.AfterSideTurnEnd: target {previousTarget?.LogName ?? "(none)"} is no longer available — " +
