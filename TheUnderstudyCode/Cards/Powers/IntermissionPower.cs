...
public class IntermissionPower : UnderstudyPower
{
    ...
    public override async Task AfterSideTurnEnd(PlayerChoiceContext context, CombatSide side, IEnumerable<Creature> participants)
    {
        ...
        foreach (var (card, _, slotSeqIdx) in planned)
        {
            if (card.Pile == null)
            {
                Log.Info($"IntermissionPower.AfterSideTurnEnd: {card.Id} is no longer in any pile — skipped");
                continue;
            }
            ...
            if (card.TargetType == TargetType.AnyEnemy && (currentTarget == null || currentTarget.IsDead))
            {
                ...
                Log.Info($"IntermissionPower.AfterSideTurnEnd: target {previousTarget?.LogName ?? "(none)"} is no longer available — " +
