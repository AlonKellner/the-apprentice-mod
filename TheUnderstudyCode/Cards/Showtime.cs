...
public class Showtime : PlayAllPlannedCard
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext context, CardPlay cardPlay)
    {
        ...
        Log.Info($"Showtime.OnPlay: playing {planned.Count} Planned slot(s)");
        ...
        foreach (var (card, _, slotSeqIdx) in planned)
        {
            ...
            if (card.Pile == null)
            {
                Log.Info($"Showtime.OnPlay: {card.Id} is no longer in any pile — skipped");
                continue;
            }
            ...
            if (card.TargetType == TargetType.AnyEnemy && (currentTarget == null || currentTarget.IsDead))
            {
                ...
                Log.Info($"Showtime.OnPlay: target {previousTarget?.LogName ?? "(none)"} is no longer available — " +
